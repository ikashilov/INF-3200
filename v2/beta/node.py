...
# All nodes except this (can be replaced by 'all_nodes.remove(node_host)')
other_nodes = []
...
def system_check():
    ...
    while True:
        ...
        if resp.status == 500:
            ...
            if successor in others:
                other_nodes.remove(successor)
...
#****************************************************************************************************
    ...
    def do_POST(self):
        ...
        if self.path == "/sim-recover":
            ...
            # Check wether we are the lat node in the system
            last_node = all_nodes[-1]

            # Redirect query to the last node in the chord
            if node_host != last_node:
                conn = httplib.HTTPConnection(last_node)
                conn.request("POST", "/join?nprime="+nprime)
                resp = conn.getresponse()
                conn.close() 
            # Handle it. We are the last one
            else:
                ...
                # amazing crutch: we  are activating frshly joined node
                in_chord = True
...
if __name__ == "__main__":
    ...
    # We assume starting in an empty state uncomment to opposite
    #own_key_space = keys
